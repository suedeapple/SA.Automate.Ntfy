...
/// <summary>
/// Umbraco Automate action that sends a notification to a protected ntfy topic using
/// the connection's access token. Supports optional title, tags, and priority.
/// </summary>
[Action("ntfy.SendAuthenticatedNotification", "Send ntfy Notification (Authenticated)",
    Description = "Sends an ntfy Notification to a protected topic",
    Group = "Messaging",
    Icon = "icon-bell",
    ConnectionTypeAlias = "ntfy-auth")]
public class SendAuthenticatedNotificationAction : ActionBase<SendNotificationSettings, SendNotificationOutput>
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<SendAuthenticatedNotificationAction> _logger;
    private readonly IOptionsMonitor<NtfySettings> _ntfySettings;

    public SendAuthenticatedNotificationAction(
        ActionInfrastructure infrastructure,
        IHttpClientFactory httpClientFactory,
        ILogger<SendAuthenticatedNotificationAction> logger,
    ...
    /// with the connection's access token, to the ntfy publish API. Returns a failed
    /// result on validation errors, missing configuration, or a non-success API response.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var connectionSettings = context.Connection?.GetSettings<NtfyAuthConnectionSettings>();

        if (connectionSettings == null
            || string.IsNullOrWhiteSpace(connectionSettings.Topic)
        ...
        try
        {
            ...
            var result = await NtfyRequestHelper.PublishAsync(
                httpClient, serverUrl, connectionSettings.Topic, connectionSettings.AccessToken, settings, cancellationToken);
